...
class QSubWindowArea(QtW.QMdiArea):
    ...
    def add_sub_window(self, widget: _T, window_title: str) -> QSubWindow[_T]:
        sub_window = QSubWindow(widget, window_title)
    ...
    def _anchor_minimized_windows(self):
...
class QSubWindow(QtW.QMdiSubWindow, Generic[_T]):
    ...
    def state(self, state: SubWindowState):
        ...
        match state:
            case SubWindowState.MIN:
                ...
                n_minimized = sum(
                    1 for sub_window in self._subwindow_area().subWindowList()
                    if sub_window.state is SubWindowState.MIN
                )
    ...
    def _mouse_hover_event(self, event_pos: QtCore.QPoint):
        ...
        if self.state is not SubWindowState.NORMAL:
            return
        current_time = timer()
        if current_time - self._last_hovered < 0.01:
            return
        self._last_hovered = timer()
        ...
        if self._current_button == QtCore.Qt.MouseButton.NoButton:
            ...
            min_size = self._widget.minimumSize().expandedTo(self._title_bar.minimumSize())
            max_size = self._widget.maximumSize()
            self._resize_state.resize_widget(self, event_pos, min_size, max_size)
    
    if TYPE_CHECKING:
        def parentWidget(self) -> QSubWindowArea: ...
...
class QSubWindowTitleBar(QtW.QFrame):
    def __init__(self, subwindow: QSubWindow[_T], title: str):
        ...
        layout.addWidget(self._minimize_button, alignment=QtCore.Qt.AlignmentFlag.AlignRight)
        layout.addWidget(self._toggle_size_button, alignment=QtCore.Qt.AlignmentFlag.AlignRight)
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent):
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            if self._subwindow.state == SubWindowState.MIN:
                ...
                return
            self._drag_position = event.globalPos() - self._subwindow.frameGeometry().topLeft()
